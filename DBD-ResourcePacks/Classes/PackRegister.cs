    ...
    public class PackRegister : INotifyPropertyChanged
    {
        ...
        void RebuildDownloadList()
        {
            ...
            if (_downloadPortraits)
                foreach (ResourcePack pack in packs.Where(pack => pack.contains.Contains(Constants.CONTAINS_PORTRAITS)))
                    if (!filtered.Contains(pack.uniqueKey))
                        filtered.Add(pack.uniqueKey);
            if (_downloadPerks)
                foreach (ResourcePack pack in packs.Where(pack => pack.contains.Contains(Constants.CONTAINS_PERKS)))
                    if (!filtered.Contains(pack.uniqueKey))
                        filtered.Add(pack.uniqueKey);
            if (_downloadItems)
                foreach (ResourcePack pack in packs.Where(pack => pack.contains.Contains(Constants.CONTAINS_ITEMS)))
                    if (!filtered.Contains(pack.uniqueKey))
                        filtered.Add(pack.uniqueKey);
            if (_downloadAddons)
                foreach (ResourcePack pack in packs.Where(pack => pack.contains.Contains(Constants.CONTAINS_ADDONS)))
                    if (!filtered.Contains(pack.uniqueKey))
                        filtered.Add(pack.uniqueKey);
            if (_downloadPowers)
                foreach (ResourcePack pack in packs.Where(pack => pack.contains.Contains(Constants.CONTAINS_POWERS)))
                    if (!filtered.Contains(pack.uniqueKey))
                        filtered.Add(pack.uniqueKey);
            if (_downloadOfferings)
                foreach (ResourcePack pack in packs.Where(pack => pack.contains.Contains(Constants.CONTAINS_OFFERINGS)))
                    if (!filtered.Contains(pack.uniqueKey))
                        filtered.Add(pack.uniqueKey);
            if (_downloadMiscUI)
                foreach (ResourcePack pack in packs.Where(pack => pack.contains.Contains(Constants.CONTAINS_MISC_UI)))
                    if (!filtered.Contains(pack.uniqueKey))
                        filtered.Add(pack.uniqueKey);
