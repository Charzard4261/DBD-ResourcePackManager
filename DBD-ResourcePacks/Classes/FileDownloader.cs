...
public class FileDownloader : IDisposable
{
    ...
    public async Task DownloadFileTaskAsync(string address, string fileName, object userToken = null)
    {
        await DownloadFileTask(address, fileName, true, userToken);
    }
	...
	private async Task DownloadFileTask( string address, string fileName, bool asyncDownload, object userToken )
	{
		downloadingDriveFile = address.StartsWith( GOOGLE_DRIVE_DOMAIN ) || address.StartsWith( GOOGLE_DRIVE_DOMAIN2 );
		if( downloadingDriveFile )
		{
        ...
        await webClient.DownloadFileTaskAsync(downloadAddress, downloadPath);
    }
